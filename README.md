...
完整记录见 **[CHANGELOG.md](CHANGELOG.md)**。最新版本 **v0.3.20**：判红覆盖——143 项断言里只有 6 条断言"门必须判红"，其余问的都是"好输入会不会绿"，而一道悄悄失去牙齿的门从绿侧不可见。现在每道离线门从同一份 fixture 驱动两次：原样退 0、改坏一处退 1 且点名，"没东西可查"钉死退 5；十道门 + lib/png 共 50 条；变异测试改哑三门抓到 5 条；selftest 143→193。
