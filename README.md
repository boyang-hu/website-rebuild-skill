...
- **零依赖工具链**——72 个 Node 脚本（51 个工序脚本与验收门 + 11 个共用库 + 10 个源码化/反推重构器），源码化之前整条流水线不装任何 npm 包
...
完整记录见 **[CHANGELOG.md](CHANGELOG.md)**。最新版本 **v0.3.17**：一份 argv 合同——`lib/cli.mjs` 让全部 57 个脚本认 `--help`（头注 + 旗标清单 + skill 版本）与 `--version`，未知旗标一律 FATAL（此前 9/57）；迁移时补齐 30 余处代码读了但头注没写的旗标；退出码约定成表；`tools/package.json` 首次钉死 babel 依赖；verify-fresh 改用项目自己的 esbuild；selftest 110→114（逐脚本扫 `--help` / 未知旗标 / 头注旗标⊆已知集）；98 个历史版本打了本地 git tag。
