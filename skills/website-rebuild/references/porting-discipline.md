...
per-frame 型为什么这么写：这类更新器在源码里**本来就守在一个集合上**，而该集合在竖切期恒为空，**守卫就是全部可达体**。守卫逐字来自源站，所以"这条路径可不可达"不是移植者猜的，是源站自己写的；一旦守卫被跨过，说明竖切期的假设破了——此时要的是**带行号的响亮失败**，而不是静默的错误画面（与 `gate-failure-modes.md` §1 的防呆同源：静默通过比红更贵）。
